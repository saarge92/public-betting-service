...
async fn main(){
    ...
    // Создаем DI контейнер и передаем туда динамический параметр (подключение к БД)
